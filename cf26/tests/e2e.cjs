...
(async () => {
  ...
  await page.evaluate(() => {
    const key = "graduation-monopoly-326-save-v1";
    const save = JSON.parse(localStorage.getItem(key));
    save.seed = 1;
    save.currentPlayer = 0;
    save.players[0].position = 0;
    save.status = "ready";
    save.useBoost = false;
  ...
  assert.ok((await page.locator("#event-kicker").textContent()).includes("北国来信"));
  assert.ok((await page.locator("#event-body").textContent()).includes("吴晨"));
  assert.ok((await page.locator("#event-card-panel").getAttribute("class")).includes("theme-north"));
  await page.screenshot({ fullPage: true });
  await page.setViewportSize({ width: 375, height: 812 });
  await page.waitForTimeout(250);
  ...
  await page.waitForTimeout(250);
  assert.equal((await page.locator("#atlas-count").textContent()).trim(), "1 / 17");
  ...
  await browser.close();
  console.log("326 E2E passed: setup, gameplay, save recovery, mobile layout, atlas modal");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
  process.exit();
