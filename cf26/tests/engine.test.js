...
test("326 roster is complete and every classmate owns one board stop", () => {
  const schoolIds = DATA.board.filter((space) => space.type === "school").map((space) => space.schoolId);
  assert.equal(DATA.classmates.length, 17);
  assert.equal(new Set(DATA.classmates.map((student) => student.id)).size, 17);
  assert.equal(new Set(DATA.classmates.map((student) => student.name)).size, 17);
  assert.equal(new Set(DATA.classmates.map((student) => student.school)).size, 16);
  assert.equal(new Set(DATA.classmates.map((student) => student.province)).size, 11);
  assert.equal(new Set(DATA.classmates.map((student) => student.note)).size, 17);
  assert.equal(new Set(DATA.classmates.map((student) => student.tag)).size, 13);
  assert.ok(DATA.classmates.every((student) => student.note.length >= 45));
  assert.equal(DATA.board.length, 30);
  assert.equal(DATA.board.filter((space) => space.type === "school").length, 17);
  assert.deepEqual([...schoolIds].sort(), DATA.classmates.map((student) => student.id).sort());
});
